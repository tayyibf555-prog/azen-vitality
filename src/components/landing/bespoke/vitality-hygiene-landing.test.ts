...
describe("VitalityHygieneLanding", () => {
  ...
  it("renders the variant-B design extras (sticky CTA bar + hero price chip), absent on A", () => {
    const b = render("b");
    // Hero price chip with its value + note, under the hero subhead. No finance wording.
    expect(b).toContain('class="vd-hero-chip"');
    expect(b).toContain('class="vd-hero-chip-value"');
    expect(b).toContain("Usually about 30 minutes");
    expect(b.toLowerCase()).not.toContain("finance");
