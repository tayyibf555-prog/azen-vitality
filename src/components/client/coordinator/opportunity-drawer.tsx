...
export function OpportunityDrawer({
  ...
  // Booking state. The picked live availability slot: Dentally refuses a booking
  // without an end time and a clinician, and only a real slot carries those, so
  // the booking is driven by a selection out of the diary, not a typed-in time.
  ...
  async function book() {
    ...
    try {
      const res = await fetch("/api/coordinator/book", {
        ...
        body: JSON.stringify({ opportunityId: opportunity.id, ...built.fields }),
      });
      ...
      setTouches((prev) => [
        ...prev,
        {
          id: `b-${prev.length}`,
          kind: "booking",
          body: `Next step booked for ${slotFullLabel(built.fields.start)}`,
          at: new Date().toISOString(),
        },
      ]);
    } catch (err) {
