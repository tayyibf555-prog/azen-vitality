...
async function authorise(
  ...
  // THE MODULE LOCK. The medical history is a tab of the patient record, so it belongs
  ...
  // THE CLINICAL-WRITE LOCK, on writes only. Recording or reviewing a medical history, or retracting one, is a clinical
...
export async function POST(
  ...
  const auth = await authorise(
    ...
    String(payload.patientId ?? ""),
    {
      ...
      capability: action === "retract" ? "clinical.record.retract" : "clinical.medical-history.write",
    },
  );
  if (auth instanceof Response) return auth;
