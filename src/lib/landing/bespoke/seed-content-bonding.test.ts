...
// Requirement: the JSON seeded by migration 0057 for BOTH variants must be valid,
...
// Mirrors the 0056 (Invisalign) seed test.

const MIGRATION_PATH = resolve(
  process.cwd(),
  "supabase/migrations/0057_vitality_bonding_landing.sql",
);
...
describe("0057 seed content", () => {
  ...
  it("the two variants differ only in their hero and CTA (the A/B surface)", () => {
    ...
    expect(a.pricing.lines[0]).toMatchObject({ treatment: "Composite bonding", fromPriceGBP: 180 });
    expect(b.pricing.lines[0]).toMatchObject({ treatment: "Composite bonding", fromPriceGBP: 180 });
  ...
  it("is seeded as a DRAFT and not published", () => {
    const sql = readFileSync(MIGRATION_PATH, "utf8");
    // The landing_page insert seeds the bonding page as a draft (goes live later).
    expect(sql).toContain("'bonding', 'bonding', null, 'draft'");
