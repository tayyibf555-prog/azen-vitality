...
describe("every nav surface arms a prefetch on intent", () => {
  ...
  // that silently removes the prefetch and nothing else changes. The two files
  // that are pure tab strips carry NO plain <Link> at all, which makes the rule
  // trivially checkable. (client-sidebar.tsx is excluded on purpose: its "Sign
  // in" link points at /login, a genuinely static route that needs no arming.)
  it.each([
    "components/client/section-tabs.tsx",
    "components/client/patients/record/patient-tab-strip.tsx",
